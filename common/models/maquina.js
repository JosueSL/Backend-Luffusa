...
module.exports = function(Maquina) {

    function query(maquinas, fecha){
        let query = '', equipos = '';
        let cont=0;
        ...
        WHERE TagName IN ('SNA061T000SHP', CONCAT('SNA',@cod,'T000SHP'), 'SOC'+@cod+'T000SHP', 'SOC'+@cod+'T100SHP', 'BAG'+@cod+'T004SHP'
            ...
            AND EventStamp BETWEEN '${fecha} 00:00:00' AND '${fecha} 23:59:59';
        SET @cont = @cont + 1
        END
        SELECT * FROM #datos`;
        return query;
    ...
    Maquina.Alarmas = (datos, cb) => {
        ...
        async function almDB() {
            ...
            await new sql.ConnectionPool(Conexion).connect().then(async pool => {
                return await pool.query(query(datos.maquinas, datos.fecha))
            }).then(async result => {
